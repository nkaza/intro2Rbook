...
```{r 4.Q4}
NC_AQI_data <- read_csv("NC_AQI_data.csv")
```
\ 
...
```{r 4.Q5}
head(NC_AQI_data) # display the first 5 rows
names(NC_AQI_data) # display the variable names
str(NC_AQI_data) # display the structure of the dataframe NC_AQI_data
```
...
all.equal(NC_AQI_data$date_2020, NC_AQI_data$dateu)

library(lubridate)
library(magrittr) ## Need this pacakge for a different pipe function than we are used to.

NC_AQI_data %<>% # This pipe function is needed to mutate in place reusing the same variable names.
  mutate(
    dateu = ymd(dateu),
    date_2020 = mdy(dateu)
  )
...
```{r 4.Q8}
AQI <- NC_AQI_data %>%
  select(ID, dateu, PM2.5_daily, temperature)
```
...
AQI_refined_March <- AQI_refined %>%
  filter(month(date_collected) != 2)
```
...
```{r 4.Q13}
AQI_PL <- AQI_refined_March %>%
  mutate(Pollution_Level = if_else((AQI > 100), "Polluted", "Normal")) %>%
  filter(Pollution_Level == "Polluted")
```
...
NC_county <- read_csv("NC_county.csv")

AQI_March_Joint <- left_join(AQI_refined_March, NC_county, by = c("sensor_ID" = "sensor_ID"))

AQI_March_Durham <- AQI_March_Joint %>%
...
```{r 4.Q15}
write_csv(AQI_March_Joint, "AQI_March_Joint.csv")
```
